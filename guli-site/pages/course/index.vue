...
export default {
  ...
  async asyncData({ route }) {
    ...
    // 调用api
    // 查询课程分类信息
    const subjectNestedTreeListResponse = await subjectApi.getSubjectNestedTreeList()
    const subjectNestedList = subjectNestedTreeListResponse.data.data.items
    let subSubjectList = []
    // 从类别嵌套列表中找到对应一级类别下的二级类别数据
    ...
    return {
      // 组装data数据
      subjectNestedList: subjectNestedList, // 一级分类
      subSubjectList: subSubjectList, // 二级分类
      searchObj: searchObj, // url查询参数
      data: courseListResponse.data.data
    }
  },

  created() {
    console.log(this.$route.query.subjectParentId)
  },
